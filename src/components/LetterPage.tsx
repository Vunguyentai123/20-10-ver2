...
const LetterPage = ({ onReadLetter }: LetterPageProps) => {
  ...
  const handleNoClick = (e: React.MouseEvent) => {
    ...
    do {
      ...
      // Nếu không va chạm với cả 2 thì ok
  ...
  return (
    <>
      ...
      {/* Buttons appear after letter stops */}
      {/* Read Letter Button - left side, symmetric from center */}
      {/* Button width ≈ 170px, đặt cạnh phải của button cách tâm 40px */}
      <div 
        className="absolute z-50 transition-opacity duration-700 ease-in-out" 
        style={{ 
          top: '50%',
          left: '50%',
      ...
      >
        <button
          ...
          className="px-8 py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-xl font-bold rounded-full shadow-2xl hover:shadow-pink-300 hover:scale-110 transition-all duration-300 whitespace-nowrap"
        >
          💌 Đọc thư
        </button>
      </div>

      {/* No Button placeholder - right side, symmetric from center (initial position) */}
      {/* Đặt cạnh trái của button cách tâm 40px */}
      {!noButtonPosition && (
        <div 
          className="absolute z-50 transition-opacity duration-700 ease-in-out" 
          style={{ 
