...
const LetterPage = ({ onReadLetter }: LetterPageProps) => {
  const [letterFalling, setLetterFalling] = useState(true);
  const [showButtons, setShowButtons] = useState(false);
  const [noButtonPosition, setNoButtonPosition] = useState<{ x: number; y: number } | null>(null);

  useEffect(() => {
    // Letter rơi trong 2.5s
    const letterTimer = setTimeout(() => {
      setLetterFalling(false);
    }, 2500);
    
    // Buttons hiện ra SAU KHI letter dừng (2.5s + delay nhỏ)
    const buttonTimer = setTimeout(() => {
      setShowButtons(true);
    }, 2800); // Letter dừng xong 0.3s thì buttons mới bắt đầu fade in
  ...
  const handleNoClick = (e: React.MouseEvent) => {
    ...
    if (isMobile) {
      // Mobile: Tránh letter và read button
      const margin = 40;
      
      // Get actual letter element position - CHỈ LẤY SVG TRONG MOBILE CONTAINER
      const mobileContainer = document.querySelector('.sm\\:hidden');
      const letterElement = mobileContainer?.querySelector('svg');
      let letterLeft = 0;
      let letterRight = 0;
      let letterTop = 0;
      let letterBottom = 0;
      ...
      if (letterElement) {
        ...
        letterLeft = letterRect.left - margin;
        letterRight = letterRect.right + margin;
        letterTop = letterRect.top - margin;
        letterBottom = letterRect.bottom + margin;
      }
      
      // Get actual Read button position - CHỈ LẤY BUTTON TRONG MOBILE CONTAINER
      let readButtonLeft = 0;
      let readButtonRight = 0;
      let readButtonTop = 0;
      let readButtonBottom = 0;
      let foundReadButton = false;
      
      if (mobileContainer) {
        const readButton = Array.from(mobileContainer.querySelectorAll('button')).find(
          btn => btn.textContent?.includes('Đọc thư') && btn !== clickedButton
        );
        
        if (readButton) {
          const rect = readButton.getBoundingClientRect();
          readButtonLeft = rect.left - margin;
          readButtonRight = rect.right + margin;
          readButtonTop = rect.top - margin;
          readButtonBottom = rect.bottom + margin;
          foundReadButton = true;
        }
      }
      
      do {
        // Random position trong viewport, tránh edges
        ...
        // Check overlap with letter
        const overlapWithLetter = letterElement ? !(
          noButtonRight < letterLeft ||
          noButtonLeft > letterRight ||
          noButtonBottom < letterTop ||
          noButtonTop > letterBottom
        ) : false;
        
        // Check overlap with read button
        const overlapWithReadButton = foundReadButton ? !(
          noButtonRight < readButtonLeft ||
          noButtonLeft > readButtonRight ||
          noButtonBottom < readButtonTop ||
          noButtonTop > readButtonBottom
        ) : false;
        
        if (!overlapWithLetter && !overlapWithReadButton) {
          break;
        }
        
      } while (attempts < maxAttempts);
    } else {
      // Desktop: Get ACTUAL positions from DOM using getBoundingClientRect
      const margin = 40;
      
      // Get actual letter element position - CHỈ LẤY SVG TRONG DESKTOP CONTAINER
      const desktopContainer = document.querySelector('.hidden.sm\\:block');
      const letterElement = desktopContainer?.querySelector('svg');
      let letterLeft = 0;
      let letterRight = 0;
      let letterTop = 0;
      let letterBottom = 0;
      ...
      // Get actual Read button position - CHỈ LẤY BUTTON TRONG DESKTOP CONTAINER
      let readButtonLeft = 0;
      let readButtonRight = 0;
      let readButtonTop = 0;
      let readButtonBottom = 0;
      ...
      if (desktopContainer) {
        const readButton = Array.from(desktopContainer.querySelectorAll('button')).find(
          btn => btn.textContent?.includes('Đọc thư') && btn !== clickedButton
        );
      ...
      do {
        ...
        // Check overlap with letter (AABB collision detection)
        const overlapWithLetter = letterElement ? !(
          noButtonRight < letterLeft ||
          noButtonLeft > letterRight ||
          noButtonBottom < letterTop ||
        ...
        // Check overlap with read button (AABB collision detection)
        const overlapWithReadButton = foundReadButton ? !(
          noButtonRight < readButtonLeft ||
          noButtonLeft > readButtonRight ||
          noButtonBottom < readButtonTop ||
  ...
  return (
    <div className="relative w-full min-h-screen overflow-y-auto overflow-x-hidden">
      {/* Mobile Layout - flex centered */}
      <div className="sm:hidden flex flex-col items-center justify-center min-h-screen gap-8 px-4">
        {/* Letter for mobile */}
        <div 
          className="z-50"
          style={{
            transform: letterFalling ? 'translateY(-150vh)' : 'translateY(0)',
            transition: 'transform 2.5s cubic-bezier(0.34, 1.56, 0.64, 1)'
          }}
        >
          <div style={{
            animation: !letterFalling ? 'gentlePulse 3s ease-in-out infinite' : 'none'
          }}>
            <svg 
            ...
            >
            <path fill="#FECC85" d="M14.216,61.793v388.413h483.569V61.793H14.216z M353.996,302.991
            ...
            <path fill="#FF5E8A" d="M309.469,205.373c-29.959,0-53.464,33.405-53.464,33.405s-22.173-33.405-53.478-33.405
            ...
            <path fill="#000" d="M497.784,47.577H14.216C6.365,47.577,0,53.942,0,61.793v79.864v304.304v4.245c0,7.852,6.365,14.216,14.216,14.216h483.568
            ...
            <path fill="#000" d="M328.734,138.282H140.847c-7.852,0-14.216-6.365-14.216-14.216s6.365-14.216,14.216-14.216h187.887
              c7.852,0,14.216,6.365,14.216,14.216S336.585,138.282,328.734,138.282z"/>
            <path fill="#000" d="M371.152,138.277c-0.924,0-1.862-0.085-2.772-0.27s-1.805-0.455-2.658-0.81c-0.867-0.355-1.692-0.796-2.459-1.308
        ...
        {/* Mobile Buttons - appear after letter stops */}
        <div className="flex flex-col gap-4 w-full max-w-sm">
          {showButtons && (
            <button
              ...
              className="min-w-[160px] px-8 py-3 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-pink-300 whitespace-nowrap mx-auto"
            >
              💌 Đọc thư
            </button>
          )}
          ...
          {showButtons && !noButtonPosition && (
            <button
              ...
              className="min-w-[160px] px-8 py-3 bg-gradient-to-r from-gray-400 to-gray-500 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-gray-300 whitespace-nowrap mx-auto"
            >
              💔 Không đọc
            </button>
          )}
      ...
      <div className="hidden sm:block">
        {/* Falling Letter - centered */}
        <div 
          className="absolute top-1/2 left-1/2 z-50"
          style={{
            transform: letterFalling 
              ? 'translateY(-150vh)' 
              : 'translate(-50%, -50%)',
            transition: 'transform 2.5s cubic-bezier(0.34, 1.56, 0.64, 1)'
          }}
        >
          <div style={{
            ...
            <svg 
              className="w-36 h-36 md:w-40 md:h-40 drop-shadow-2xl" 
              viewBox="0 0 512 512" 
              xmlns="http://www.w3.org/2000/svg"
            >
            <path fill="#FECC85" d="M14.216,61.793v388.413h483.569V61.793H14.216z M353.996,302.991
        ...
        {/* Desktop Buttons */}
        {/* Read Letter Button - Desktop only - Left side */}
        {showButtons && (
          <div
          ...
          >
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-pink-300 whitespace-nowrap"
            >
              💌 Đọc thư
            </button>
          </div>
        ...
        {/* No Button - Desktop only - Right side */}
        {showButtons && !noButtonPosition && (
          <div
            className="absolute z-50"
            style={{
          ...
          >
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-gray-400 to-gray-500 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-gray-300 whitespace-nowrap"
            >
              💔 Không đọc
            </button>
          </div>
      ...
      {/* No Button - moves on click (flying position) - works for both mobile and desktop */}
      {noButtonPosition && (
        <button
          onClick={handleNoClick}
          onMouseEnter={(e) => {
          ...
          className="min-w-[160px] px-8 py-3 bg-gradient-to-r from-gray-400 to-gray-500 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-gray-300 whitespace-nowrap"
          style={{
            position: 'fixed',
